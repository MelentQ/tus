  ...
  const callbackMap = function (entries, observer) {
    entries.forEach((entry) => {
      if (entry.isIntersecting) {
        ...
        console.log(targetMap.dataset);
        script.src = `https://api-maps.yandex.ru/2.1/?apikey=${targetMap.dataset.key}&lang=ru_RU`;
        document.body.append(script);
        const icon = targetMap.dataset.icon !== undefined ? targetMap.dataset.icon : "../img/icon-map.svg";
          ...
          function init() {
            ymaps.ready(function () {
              const myMap = new ymaps.Map(targetMap, {
                ...
                zoom: targetMap.dataset.zoom !== undefined ? targetMap.dataset.zoom : 13,
              });
            });
          }
        });
